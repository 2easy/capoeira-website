class ::NewsListRenderer < WillPaginate::ViewHelpers::LinkRenderer
  ...
  def summary
    tag(:span,
        @options[:summary_label] % [ @collection.offset + 1, @collection.offset + @collection.length, @collection.total_entries ],
        :class => "summary"
       )
  end
end
